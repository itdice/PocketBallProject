...
#define NICKNAME "SEOUL21_IT_DICE_TaeHyun"
...
typedef struct scoreBlock {
	...
	double angle;
	double power;
	bool operator<(scoreBlock other) {
...
constexpr double HALF_DIAMETER = 5.73 / 2;
...
constexpr double C_DISTANCE = 1;
constexpr double C_ANGLE = 10.0;
constexpr double C_OBSTACLE = 100.0;
constexpr double MAX_SCORE = 1000.0;
constexpr double MAX_WIDTH = 254.0;

float lineFunction(float incline, const Pos point, const int curX) {
	return point.y + (incline * (curX - point.x));
}

bool hasBallInPath(const Pos start, const Pos target, Pos& ballPos) {
	float minX, maxX;
	float upperY, lowerY;
	float maxUpperY, minUpperY, maxLowerY, minLowerY;

	if (target.y == start.y) {
		minX = min(target.x, start.x);
		maxX = max(target.x, start.x);
		upperY = target.y + 2 * HALF_DIAMETER;
		lowerY = target.y - 2 * HALF_DIAMETER;
		if (ballPos.x <= maxX && ballPos.x >= minX && ballPos.y >= lowerY && ballPos.y <= upperY)
			return true;
	}
	else if (target.x == start.x) {
		minX = target.x - 2 * HALF_DIAMETER;
		maxX = target.x + 2 * HALF_DIAMETER;
		lowerY = min(target.y, start.y);
		upperY = max(target.y, start.y);
		if (ballPos.x <= maxX && ballPos.x >= minX && ballPos.y >= lowerY && ballPos.y <= upperY)
			return true;
	}
	else {
		float incline = (target.y - start.y) / (target.x - start.x);

		float width = fabsf(target.x - start.x);
		float height = fabsf(target.y - start.y);
		double radian = height > 0 ? atan(width / height) : 0;

		float dx = HALF_DIAMETER * sin(radian);
		float dy = HALF_DIAMETER * cos(radian);

		minX = min(start.x, target.x);
		maxX = max(start.x, target.x) + 2 * dx;
		float minY = (minX == start.x) ? start.y : target.y;

		if (target.y < start.y) {
			maxUpperY = start.y + 2 * dy;
			maxLowerY = start.y - 2 * dy;
			minUpperY = target.y + 2 * dy;
			minLowerY = target.y - 2 * dy;
		}
		else {
			maxUpperY = target.y + 2 * dy;
			maxLowerY = target.y - 2 * dy;
			minUpperY = start.y + 2 * dy;
			minLowerY = start.y - 2 * dy;
		}
		//int map[200][200] = { 0, };

		//for (int startX = floor(minX - 2 * dx); startX <= maxX; startX++) {

		//	if (incline > 0) {
		//		upperY = line_function(incline, { minX - (2 * dx), minY + (2 * dy) }, startX);
		//		lowerY = line_function(incline, { minX + (2 * dx), minY - (2 * dy) }, startX);
		//	}
		//	else {
		//		upperY = line_function(incline, { minX + (2 * dx), minY + (2 * dy) }, startX);
		//		lowerY = line_function(incline, { minX - (2 * dx), minY - (2 * dy) }, startX);
		...
		//	//if (upperY >= 200 || upperY < 0 || lowerY >= 200 || lowerY < 0) continue;

		//	if (upperY <= maxUpperY && upperY >= minUpperY)
		//		map[(int)floor(upperY)][startX] = 1;

		//	if (lowerY <= maxLowerY && lowerY >= minLowerY)
		//		map[(int)floor(lowerY)][startX] = 1;
		//}

		// answer
		if (incline > 0) {
			upperY = lineFunction(incline, { minX - (2 * dx), minY + (2 * dy) }, ballPos.x);
			lowerY = lineFunction(incline, { minX + (2 * dx), minY - (2 * dy) }, ballPos.x);
		}
		else {
			upperY = lineFunction(incline, { minX + (2 * dx), minY + (2 * dy) }, ballPos.x);
			lowerY = lineFunction(incline, { minX - (2 * dx), minY - (2 * dy) }, ballPos.x);
		}
		if (upperY <= maxUpperY && upperY >= minUpperY && lowerY <= maxLowerY && lowerY >= minLowerY)
			return true;


		//// Plot 
		//int curY = floor(ballPos.y);
		//int curX = floor(ballPos.x);
		//map[curY][curX] = 2;

		//map[(int)floor(start.y)][(int)floor(start.x)] = 3;
		//map[(int)floor(target.y)][(int)floor(target.x)] = 4;
...
int main()
{
	...
	while (1)
	{
		...
		ScoreBlock scores[12];
		int score_idx = 0;
		double score;
		/* score
		* difference angle * 10
		* difference dist * 1
		* another ball in path * 100
		*/

		Pos white = danggoos[0];
		for (const auto& targetIndex : orderList[order]) {
			// 1,3 or 2,4 번을 안치고 index가 현재 5인 경우
			if (danggoos[orderList[order][0]].x != -1 || danggoos[orderList[order][1]].x != -1) {
				if (targetIndex == 5) continue;
			}
			...
			for (const auto& pocket : pockets) {
				...
				if (isPossibleAngle(hitAngle, targetToHallAngle)) {
					...
					// ***********************************
					int numberBallInPath = 0;
					for (const auto& avoidTargetIndex : orderList[ohterOrder]) {
						if (avoidTargetIndex == targetIndex) continue;
						if (hasBallInPath(white, hitPoint, danggoos[avoidTargetIndex])) {
							numberBallInPath++;
						}
					}
					// ***********************************
					...
					score = C_DISTANCE * (distanceToTarget + distanceToHall) + C_ANGLE * abs(targetToHallAngle - pointAngle) + C_OBSTACLE * numberBallInPath;
					scores[score_idx] = { score, angle, power };
				}
				else {
					// 모든 홀로 갈 수 없는 상황인 경우
					double constAngle;
					if (angleToCenter(white, target) <= 90) {
						constAngle = NOMINAL_ANGLE;
					}
					else if (angleToCenter(white, target) <= 180) {
						constAngle = NOMINAL_ANGLE + 90;
					}
					else if (angleToCenter(white, target) <= 270) {
						constAngle = NOMINAL_ANGLE + 180;
					}
					else {
						constAngle = NOMINAL_ANGLE + 270;
					}
					Pos hitPoint = getHitPoint(target, constAngle);
					...
					score = C_DISTANCE * (distanceToTarget + MAX_WIDTH) + C_ANGLE * (abs(whiteToHitPointAngle - constAngle)) + C_OBSTACLE * numberBallInPath;
					scores[score_idx] = { score, angle, power };
				}
				score_idx++;
			}
		...
		for (int i = 0; i < score_idx; i++) {
			printf("%dth score: %lf angle: %lf power: %lf,\n", i, scores[i].score, scores[i].angle, scores[i].power);
		}

		std::sort(scores, scores + score_idx);
		angle = scores[0].angle;
		power = scores[0].power;
